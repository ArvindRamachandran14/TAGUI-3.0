...
class CtrlMon2(Frame) :
    ...
    def buildContent(self) :
        ...
        self.scale1 = Scale(self, from_=min(self.slider_list), to=max(self.slider_list), command=self.scale_value_check1, orient=HORIZONTAL)#, command=set_plot_range(1))
        ...
        self.scale2 = Scale(self, from_=min(self.slider_list), to=max(self.slider_list), command=self.scale_value_check2, orient=HORIZONTAL)#, command=set_plot_range(2))
        ...
        self.scale3 = Scale(self, from_=min(self.slider_list), to=max(self.slider_list), command=self.scale_value_check3, orient=HORIZONTAL)#, command=set_plot_range(3))
        ...
        # 
        self.fig4 = Figure(figsize=(4, 3))
        self.ax4 = self.fig1.add_subplot(111)
        self.ax4.set_xlabel('Time (sec)')
        self.ax4.set_ylabel('pCO2 (ppm)')
        self.ax4.set_autoscalex_on(True)
        self.ax4.set_ybound(0, 10000)
        self.ax4.set_autoscaley_on(False)
        self.ax4.grid(True, 'major', 'both')
        #plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
        self.fig4.tight_layout()
        self.cnvs4 = FigureCanvasTkAgg(self.fig1, self)
        self.cnvs4.get_tk_widget().grid(row=3, column=0)
        #self.cnvs1.get_tk_widget().pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # Plot Partial pressure of H2O
       
        self.fig5 = Figure(figsize=(4, 3))
        self.ax5 = self.fig2.add_subplot(111)
        self.ax5.set_xlabel('Time (sec)')
        self.ax5.set_ylabel('pH20 (Pa)')
        self.ax5.set_autoscalex_on(True)
        #self.ax2.set_ybound(0, 50)
        self.ax5.set_autoscaley_on(True)
        self.ax5.grid(True, 'major', 'both')
        #plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
        self.fig5.tight_layout()
        self.cnvs5 = FigureCanvasTkAgg(self.fig2, self)
        self.cnvs5.get_tk_widget().grid(row=3, column=1)
        #self.cnvs2.get_tk_widget().pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # Plot Sample weight
        
        self.fig6 = Figure(figsize=(4, 3))
        self.ax6 = self.fig3.add_subplot(111)
        self.ax6.set_xlabel('Time (sec)')
        self.ax6.set_ylabel('Weight (g)')
        self.ax6.set_autoscalex_on(True)
        #self.ax3.set_ybound(0, 10)
        self.ax6.set_autoscaley_on(True)
        self.ax6.grid(True, 'major', 'both')
        self.fig6.tight_layout()
        self.cnvs6 = FigureCanvasTkAgg(self.fig3, self)
        #self.cnvs3.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        self.cnvs6.get_tk_widget().grid(row=3, column=2)
